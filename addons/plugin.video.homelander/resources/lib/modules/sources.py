...
class sources:
    ...
    def sourcesProcessTorrents(self, torrent_sources):#adjusted Fen code
        try:
            ...
            #unchecked
            uncheckedRDSources = [dict(i.items()) for i in torrent_sources if (not any(v in i.get('info_hash') for v in cachedRDHashes) and i.get('debrid', '') == 'Real-Debrid')]
            uncheckedTorrents.extend(uncheckedRDSources)
            uncheckedADSources = [dict(i.items()) for i in torrent_sources if (not any(v in i.get('info_hash') for v in cachedADHashes) and i.get('debrid', '') == 'AllDebrid')]
            uncheckedTorrents.extend(uncheckedADSources)
            uncheckedPMSources = [dict(i.items()) for i in torrent_sources if (not any(v in i.get('info_hash') for v in cachedPMHashes) and i.get('debrid', '') == 'Premiumize.me')]
            uncheckedTorrents.extend(uncheckedPMSources)
            uncheckedDLSources = [dict(i.items()) for i in torrent_sources if (not any(v in i.get('info_hash') for v in cachedDLHashes) and i.get('debrid', '') == 'Debrid-Link.fr')]
            uncheckedTorrents.extend(uncheckedDLSources)
            for i in uncheckedTorrents: i.update({'source': 'unchecked torrent'})

            return cachedTorrents + uncheckedTorrents
        except:
    ...
    def sourcesSort(self):
        ...
        remove_unchecked = control.setting('remove.unchecked') or 'false'
        ...
        for d in debrid.debrid_resolvers:
            ...
            if torrentSources:
                ...
                if check_torr_cache == 'true' and d.name in cache_check_resolvers:
                    ...
                    if (remove_unchecked == 'false' or len(cached) == 0 or self.unfiltered) and not autoplay_on:
                        filter += [dict(i.items()) for i in checkedTorrentSources if i['source'] == 'unchecked torrent']
                else:
    ...
    def sourcesDirect(self, items):
        ...
        filter = [i for i in items if 'unchecked' in i['source'].lower()]
        items = [i for i in items if not i in filter]
